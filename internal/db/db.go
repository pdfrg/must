...
func (ld *LibraryDB) GetAllArtists() ([]string, error) {
	rows, err := ld.db.Query(`SELECT DISTINCT artist FROM tracks WHERE artist != '' ORDER BY artist`)
	if err != nil {
...
func (ld *LibraryDB) GetAlbumsByArtist(artist string) ([]string, error) {
	rows, err := ld.db.Query(`SELECT DISTINCT album FROM tracks WHERE artist = ? AND album != '' ORDER BY album`, artist)
	if err != nil {
