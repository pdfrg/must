...
func (n *NowPlaying) UpdateStyles(styles *config.ThemeStyles, accentColor, cursorColor, progressTrackColor string) {
	...
	n.progress = buildProgress(n.effectiveProgressContainerWidth(), n.accentColor, emptyColor)
}
...
func (n *NowPlaying) effectiveProgressContainerWidth() int {
	if n.contentWidth > 0 {
...
func (n NowPlaying) progressDetailsLine(data NowPlayingData) string {
	...
	if rightWidth >= innerWidth {
		return " " + ansi.Truncate(right, innerWidth, "…")
	}
	if leftWidth+rightWidth+1 > innerWidth {
		left = ansi.Truncate(left, max(innerWidth-rightWidth-1, 1), "…")
		leftWidth = lipgloss.Width(left)
	}
	return " " + left + strings.Repeat(" ", max(innerWidth-leftWidth-rightWidth, 1)) + right
}
...
func (n NowPlaying) renderIdleView(data NowPlayingData) string {
	...
	lines := []string{" " + title, " " + artist, " " + album, ""}
	if n.artworkGap > 0 {
		lines = append(lines, make([]string, n.artworkGap)...)
	}
	lines = append(lines, " "+progView, timeStr, "", " "+modeLine)
