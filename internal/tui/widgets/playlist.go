...
func (p Playlist) layoutColumns(albumMultiDisc map[string]bool) []playlistColumn {
	...
	fixedWidth := 1 + len(p.columns)
	flexCount, totalWeight := 0, 0
	...
	flexibleWidth := max(p.width-fixedWidth, flexCount*minFlexWidth)
	remainingExtra := flexibleWidth - flexCount*minFlexWidth
