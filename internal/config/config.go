...
type Config struct {
	...
	MouseEnabled          bool                  `toml:"mouse_enabled" comment:"enable mouse interaction in the library browser (default: true)"`
	MouseFocusOnHover     bool                  `toml:"mouse_focus_on_hover" comment:"focus a library browser column when the pointer moves over it (default: true)"`
...
func GetCtlSocketPath() string {
	runtimeDir := os.Getenv("XDG_RUNTIME_DIR")
	if runtimeDir == "" {
		runtimeDir = os.TempDir()
	}
	return filepath.Join(runtimeDir, "must", "ctl.sock")
}
